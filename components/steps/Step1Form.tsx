...
export function Step1Form({ onSubmit, initialData }: Step1FormProps) {
  ...
  const [selectedCrop, setSelectedCrop] = useState(initialData?.crop || '');
  const [showCropDropdown, setShowCropDropdown] = useState(false);

  const [landSize, setLandSize] = useState(initialData?.landSize || 1);
  const [ownership, setOwnership] = useState<'owned' | 'tenant' | 'leasehold' | ''>(
    initialData?.ownership || ''
  );
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-straw p-6 shadow-sm">
        <div className="space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-soil/60 font-tamil">
              {t('form.farmerName.labelTa')}
            </p>
            <input
              type="text"
              value={farmerName}
              ...
              className="w-full h-11 px-3 border border-straw rounded-lg bg-white focus:border-turmeric focus:ring-2 focus:ring-turmeric/20 outline-none transition-all text-soil text-sm"
            />
          </div>
          ...
          <div className="space-y-1.5" ref={districtRef}>
            ...
            <p className="text-xs text-soil/60 font-tamil">
              {t('form.district.labelTa')}
            </p>
            <div className="relative">
              <button
                type="button"
                onClick={() => setShowDistrictDropdown(!showDistrictDropdown)}
                className="w-full h-11 px-3 border border-straw rounded-lg bg-white flex items-center justify-between hover:border-turmeric/50 transition-all"
              >
                <div className="flex items-center gap-2">
                  {selectedDistrictData && (
                    <Check className="w-4 h-4 text-paddy stroke-[2.5]" />
                  )}
                  <span className={`text-sm ${selectedDistrictData ? 'text-soil font-medium' : 'text-soil/40'}`}>
                    {selectedDistrictData
                    ...
                    <div className="max-h-48 overflow-y-auto custom-scrollbar">
                      {filteredDistricts.length > 0 ? (
                        filteredDistricts.map((d) => (
                          ...
                          >
                            <span>
                              {d.en} <span className="text-soil/60 font-tamil">— {d.ta}</span>
                            </span>
                            {selectedDistrict === d.id && <Check className="w-4 h-4 text-paddy" />}
                          </button>
                        ))
                      ) : (
                        <p className="p-3 text-xs text-soil/40 text-center">
            ...
            <div className="relative">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-sm ${selectedCropData ? 'text-soil font-medium' : 'text-soil/40'}`}>
                    {selectedCropData
                    ...
                    <div className="max-h-48 overflow-y-auto custom-scrollbar">
                      {filteredCrops.length > 0 ? (
                        filteredCrops.map((c) => (
                          ...
                          >
                            <span>
                              {c.en} <span className="text-soil/60 font-tamil">({c.ta})</span>
                            </span>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-3">
              <motion.button
                ...
                className="w-11 h-11 flex items-center justify-center border border-straw rounded-lg bg-white hover:bg-straw/30 disabled:opacity-40 disabled:cursor-not-allowed transition-all text-soil"
              >
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-11 h-11 flex items-center justify-center border border-straw rounded-lg bg-white hover:bg-straw/30 transition-all text-soil"
              >
                <Plus className="w-4 h-4 stroke-[2.5]" />
              </motion.button>
            </div>
            <p className="text-xs text-soil/50 mt-1">
              {t('form.landSize.helper')}
            </p>
          </div>

          {/* Ownership Selection Grid */}
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-soil">
              {t('form.ownership.label')}
            </label>
            <p className="text-xs text-soil/60 font-tamil">
              {t('form.ownership.labelTa')}
            </p>
          ...
          >
            {isLoading ? (
              <span className="flex items-center justify-center gap-2">
                ...
                <span>{isTamil ? 'சரிபார்க்கிறது...' : 'Checking...'}</span>
              </span>
            ) : (
              <>
